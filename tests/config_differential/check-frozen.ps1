# Fails when a file frozen.tsv records has changed: the oracle, a core source it compiles, or an
# input. The oracle and those sources are what the differential test holds the import to, and a
# changed input is a different test.
$ErrorActionPreference = 'Stop'
$repo = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)
$record = Join-Path $PSScriptRoot 'frozen.tsv'
# .NET's SHA256 rather than Get-FileHash: Windows PowerShell started from pwsh, as CI starts it,
# inherits pwsh's module path and cannot load the module Get-FileHash lives in.
